  ...
  <div class="flex">
    ...
    <div class="flex-1">
      <div class="container mx-auto p-8">
        ...
        <h1 class="text-4xl mb-4">Edit Schedule</h1>

        <form action="{{ route('schedule.edit', $schedule['id']) }}" method="POST" class="max-w-lg bg-white p-6 rounded-lg shadow-md">
          @csrf
          <div class="mb-4">
            ...
            <input
                ...
                value="{{ old('title') ?? $lasttitle }}"
            >
            <input type="hidden" name="playid" id="play-id" value="{{ old('playid') ?? $schedule['playid'] }}">
          ...
          <div class="mb-4">
            ...
            <input type="date" name="date" id="date" class="w-full p-2 border rounded-md" value="{{ old('date') ?? $schedule['date'] }}">
            @error('date') <p class="text-red-500 text-xs italic">{{ $message }}</p> @enderror
        </div>
    
        <div class="mb-4">
            <label for="time" class="block text-gray-700 text-sm font-bold mb-2">Time:</label>
            <input type="text" name="time" id="time" class="w-full p-2 border rounded-md" placeholder="Enter Time (HH:MM)" pattern="^(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$" value="{{ old('time') ?? $schedule['time_start']}}">
          ...
          <div class="mb-4">
            ...
            <select name="theater" id="theater" class="w-full p-2 border rounded-md">
              <option value="A" @if (old('theater') ?? $schedule['theater'] == "A") selected @endif>Theater A</option>
              <option value="B" @if (old('theater') ?? $schedule['theater'] == "B") selected @endif>Theater B</option>
              <option value="C" @if (old('theater') ?? $schedule['theater'] == "C") selected @endif>Theater C</option>
            </select>
          </div>
            <button type="submit" class="bg-yellow-500 text-white p-2 rounded-md hover:bg-yellow-800">Edit Schedule</button>
          </div>
        </form>
      </div>
    </div>
...
<script>
    ...
    document.EditEventListener('click', function(event) {
        const suggestionsContainer = document.getElementById('suggestions');
