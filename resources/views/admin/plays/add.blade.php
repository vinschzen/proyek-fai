      ...
      <div class="container mx-auto p-8">
        <nav class="mb-4">
          <ol class="list-none p-0 inline-flex">
            ...
            <li class="flex items-center">
              <span class="text-gray-700">Add Play</span>
            </li>
          </ol>
        </nav>

        <h1 class="text-4xl mb-4">Add Play</h1>

        <form action="{{ route('plays.store') }}" method="POST" enctype="multipart/form-data" class="max-w-lg bg-white p-6 rounded-lg shadow-md">
          @csrf
          ...
          <div class="mb-4">
            ...
            <input type="text" name="title" id="title" class="w-full p-2 border rounded-md" placeholder="Enter Title" value="{{ old('title') }}">
          ...
          <div class="mb-4">
            ...
            <input type="text" name="duration" id="duration" class="w-full p-2 border rounded-md" placeholder="Enter Duration" value="{{ old('duration') }}">
          ...
          <div class="mb-4">
            ...
            <select name="age_rating" id="age_rating" class="w-full p-2 border rounded-md">
              <option value="G" @if (old('age_rating') == "G") selected @endif>General (G)</option>
              <option value="PG" @if (old('age_rating') == "PG") selected @endif>Parental Guidance (PG)</option>
              <option value="PG-13" @if (old('age_rating') == "PG-13") selected @endif>Parents Strongly Cautioned (PG-13)</option>
              <option value="R" @if (old('age_rating') == "R") selected @endif>Restricted (R)</option>
              <option value="NC-17" @if (old('age_rating') == "NC-17") selected @endif>Adults Only (NC-17)</option>
            </select>
          </div>
          ...
          <div class="mb-4">
            ...
            <div class="mb-2">
              ...
              <input type="text" name="director" id="director" class="w-full p-2 border rounded-md" placeholder="Enter Director's Name" value="{{ old('director') }}"">
            ...
            <div id="casts-container">
              <div class="mb-2">
                <label for="casts[]" class="block text-gray-700 text-sm font-bold mb-2">Cast Member:</label>
                  @if (old('casts', []) )
                    @foreach(old('casts', []) as $index => $cast)
          ...
          <div class="text-center">
            <button type="submit" class="bg-blue-500 text-white p-2 rounded-md">Add Play</button>
          </div>
        </form>
      </div>
    </div>
